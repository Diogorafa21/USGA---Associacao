...
export async function getListaInscritosEvento(eventoId) {
  const { data, error } = await supabase
    .from('lista_inscritos_evento')
    .select('nome, pais, equipa, dorsal, data_inscricao, data_confirmacao, estado_inscricao')
    .eq('evento_id', eventoId)
    .order('data_inscricao', { ascending: true })
  return { data, error }
}

export function subscreverInscricoesEvento(eventoId, callback) {
  const channel = supabase
    .channel(`inscricoes-evento-${eventoId}`)
    .on(
      'postgres_changes',
      { event: '*', schema: 'public', table: 'inscricoes_evento', filter: `evento_id=eq.${eventoId}` },
      callback
    )
    .subscribe()
